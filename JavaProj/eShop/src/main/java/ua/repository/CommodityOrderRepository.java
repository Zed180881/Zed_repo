...
public interface CommodityOrderRepository extends
    ...
    @Query(value = "SELECT co FROM CommodityOrder co LEFT JOIN FETCH co.user LEFT JOIN FETCH co.orderStatus LEFT JOIN FETCH co.commodities com ORDER BY co.id", countQuery = "SELECT count(co.id) FROM CommodityOrder co")
    ...
    @Query("SELECT DISTINCT co FROM CommodityOrder co LEFT JOIN FETCH co.user LEFT JOIN FETCH co.orderStatus LEFT JOIN FETCH co.commodities com LEFT JOIN FETCH com.category LEFT JOIN FETCH com.producer LEFT JOIN FETCH com.commodityStatus WHERE co.user.id=:id ORDER BY co.orderDate DESC")
    List<CommodityOrder> findByUserID(@Param("id") int id);
}
